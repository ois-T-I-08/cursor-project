...
void main() {
  group('OptimizeGrowthRouteUseCase', () {
    ...
    test('same input produces same output', () {
      ...
      expect(
        route1.days.map((d) => d.date).toList(),
        route2.days.map((d) => d.date).toList(),
      );
    ...
    test('weekday-limited option placed on correct day', () {
      ...
      final wkMap = {
        'mat_tue': {3},
      }; // material available on Wednesday
      ...
      final wedActions =
          routeWed.days.isNotEmpty ? routeWed.days.first.actions : [];
      ...
      final monActions =
          routeMon.days.isNotEmpty ? routeMon.days.first.actions : [];
      final hasWeekdayItem = monActions.any(
        (a) => a.actionType == 'weekdayMaterial',
      );
    ...
    test('daily budget respects resin limit', () {
      ...
      final firstDayCount =
          route.days.isNotEmpty ? route.days.first.actions.length : 0;
    ...
    test(
      'weekday-limited item not placed on unavailable weekday as generalMaterial',
      () {
        final options = [
          const UpgradeOption(
            optionId: 'talent_tue',
        ...
        if (monDay != null) {
          final hasTalent = monDay.actions.any(
            (a) => a.optionId == 'talent_tue',
          );
          expect(
            hasTalent,
            isFalse,
            reason:
                'Weekday-limited option must not appear as any action type on wrong day',
          );
          // Non-limited item should appear
          final hasNonLimited = monDay.actions.any(
            (a) => a.optionId == 'non_limited',
          );
  ...
  group('GenerateTeamGrowthPriorityUseCase', () {
    ...
    test('4 members ranked by score', () {
      final chars = List.generate(
        4,
        (i) => _testChar('id$i', level: 20 + i * 20),
      );
    ...
    test('characters with upgrade options get higher priority', () {
      ...
      final report = const GenerateTeamGrowthPriorityUseCase()(
        team: team,
        snapshot: snapshot,
        upgradeOptionsByCharacter: opts,
      );
    ...
    test('unowned character is deprioritized', () {
      ...
      final unowned = report.memberPriorities.firstWhere(
        (p) => p.characterId == 'c2',
      );
    ...
    test('deterministic output for same input', () {
      ...
      expect(
        r1.memberPriorities.map((p) => p.characterId).toList(),
