...
class ProgressDao extends DatabaseAccessor<DriftAppDatabase>
  ...
  Future<void> upsertProgress(UserProgress p) async {
    await into(userProgressTable).insertOnConflictUpdate(_progressCompanion(p));
  }

  Future<UserProgress?> getProgress(String userId, String characterId) async {
    final row =
        await (select(userProgressTable)..where(
  ...
  Future<List<UserProgress>> getAllProgress(String userId) async {
    final rows =
        await (select(userProgressTable)
  ...
  Future<UserProgress> getOrCreateProgress(
    ...
    await into(
      userProgressTable,
    ).insert(_progressCompanion(created), mode: InsertMode.insertOrIgnore);
  ...
  Future<void> deleteAllProgressForUser(String userId) async {
    await (delete(userProgressTable)
