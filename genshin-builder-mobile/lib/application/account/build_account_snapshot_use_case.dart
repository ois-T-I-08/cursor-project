  ...
  Future<AccountSnapshot> call() async {
    ...
    final snapshots =
        characters.map((mc) {
          ...
          final talentSkill =
              hoyoChar?.talentSkill ?? progress?.talentSkill ?? 1;
          final talentBurst =
              hoyoChar?.talentBurst ?? progress?.talentBurst ?? 1;
          final weaponId = hoyoChar?.weaponId ?? progress?.weaponId;
          final weaponName = hoyoChar?.weaponName ?? progress?.weaponName;
          final weaponLevel =
              hoyoChar?.weaponLevel ?? progress?.weaponLevel ?? 1;
          final weaponRefinement =
              hoyoChar?.weaponRefinement ?? progress?.weaponRefinement ?? 1;
          final artifactCompletionVal =
              hoyoChar?.artifactCompletion ?? _calcArtifactCompletion(progress);
          ...
          return CharacterSnapshot(
            ...
            artifactCompletionAvailable:
                hoyoChar?.artifactCompletion != null ||
                progress?.artifactCompleted != null,
            memo: progress?.memo,
          );
        }).toList();
