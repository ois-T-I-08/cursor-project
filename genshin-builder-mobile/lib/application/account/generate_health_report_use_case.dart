  ...
  AccountHealthReport call({
      ...
      AccountHealthCategory(
        ...
        reasons:
            levelEval
                ...
                : ['所持キャラがありません'],
        improvementHints:
            levelScore < 50 && levelEval ? ['主要なキャラを Lv.80以上まで育てましょう'] : [],
      ...
      AccountHealthCategory(
        ...
        reasons:
            talentEval
                ...
                : ['所持キャラがありません'],
        improvementHints:
            talentScore < 40 && talentEval ? ['メインキャラの主要天賦を上げましょう'] : [],
      ),
    );
    ...
    categories.add(
      AccountHealthCategory(
        ...
        reasons:
            weaponEval ? ['$weaponChars 体の武器が Lv.80以上です'] : ['所持キャラがありません'],
      ),
    );
    ...
    if (artifactAvailable && totalOwned > 0) {
      artifactChars =
          owned
              .where(
                (c) =>
                    c.artifactCompletionAvailable &&
                    c.artifactCompletion >= 0.8,
              )
              .length;
    ...
    categories.add(
      AccountHealthCategory(
        ...
        improvementHints:
            !artifactAvailable ? ['キャラ詳細の聖遺物項目で装備を登録すると完成度を評価できます'] : [],
      ),
    );
    ...
    final effectiveScore =
        totalWeight > 0 ? (weightedSum / totalWeight).clamp(0.0, 100.0) : null;

    // Strengths & improvements from evaluated categories only
    final strengths =
        categories
            .where((c) => c.evaluated && c.normalizedScore >= 70)
            .map((c) => c.name)
            .toList();
    final improvements =
        categories
            .where((c) => c.evaluated && c.normalizedScore < 40)
            .map((c) => c.name)
            .toList();

    // Data coverage (separate from health score)
    final dataCoverage =
        snapshot.completeness == DataCompleteness.complete
            ? '高'
    ...
    return AccountHealthReport(
      ...
      rating:
          effectiveScore != null
              ? AccountHealthReport.scoreToRating(effectiveScore)
