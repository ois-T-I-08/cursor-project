...
describe("TeamRecommendationService", () => {
  it("completes with observed/rule recommendations from AZA and rules", async () => {
    ...
    expect(completed?.result?.engine).toBe("aza+rules");
    expect(completed?.result?.recommendations[0]).toMatchObject({ simulationStatus: "observed" });
    expect(completed?.result?.warning).toBeUndefined();
  });

  it("marks abyssUnavailable when abyss data is empty", async () => {
    const store = new MemoryStore();
    ...
    expect(result?.result?.warning).toBe("abyssUnavailable");
  ...
  it("bounds process-local active jobs", async () => {
    let releaseAbyss: (() => void) | undefined;
    const store = new MemoryStore();
    const service = new TeamRecommendationService(
      store,
      () => new Promise<AbyssStatistics>((resolve) => {
        releaseAbyss = () => resolve(abyss());
      }),
...
function build(characterId: string, weaponId: string): TeamRecommendationRequest["characters"][number] {
  return {
    characterId, element: "hydro", rarity: 5, isOwned: true, level: 90, ascension: 6, constellation: 0,
    talents: { normal: 9, skill: 9, burst: 9 }, weapon: { weaponId, level: 90, ascension: 6, refinement: 1 },
    artifacts: { sets: [], stats: {} }, inputQuality: "exact", defaultedFields: [],
  };
}
function abyss(): AbyssStatistics {
  return {
    ...
    teams: [{
      half: "upper",
      members: request.characters.map((value) => value.characterId),
      usageRate: 0.1,
      ownershipRate: 0.2,
      usageAmongOwnersRate: 0.5,
    }],
  };
}
...
class MemoryStore implements SimulationStore {
  ...
  createdJobs = 0;
  async deleteExpiredJobs(current: Date) {
    for (const [id, row] of this.jobs) if (row.expiresAt <= current) this.jobs.delete(id);
  }
  async findReusableJob(requestHash: string, current: Date) {
    return [...this.jobs.values()].find((row) =>
      row.hash === requestHash && row.expiresAt > current && ["queued", "running", "completed"].includes(row.job.status))?.job ?? null;
  }
  async createJob(input: { jobId: string; requestHash: string; attackerId: string; expiresAt: Date }) {
    this.createdJobs += 1;
    this.jobs.set(input.jobId, { job: { jobId: input.jobId, status: "queued" }, hash: input.requestHash, expiresAt: input.expiresAt });
  }
  async setJobStatus(jobId: string, status: JobStatus) {
    this.row(jobId).job = { jobId, status };
  }
  async completeJob(jobId: string, result: TeamRecommendationResult) {
    this.row(jobId).job = { jobId, status: "completed", result };
  }
  async failJob(jobId: string, errorCode: NonNullable<TeamRecommendationJob["errorCode"]>) {
    this.row(jobId).job = { jobId, status: "failed", errorCode };
  }
  async readJob(jobId: string, current: Date) {
    const row = this.jobs.get(jobId);
    return !row ? null : row.expiresAt <= current ? { jobId, status: "expired" as const } : row.job;
  }
