...
const DEFAULT_WEIGHTS: ScoreWeights = { observed: 0.3, build: 0.25, stability: 0.15, energy: 0.15, accessibility: 0.15 };
...
function readWeights(env: NodeJS.ProcessEnv = process.env): ScoreWeights {
  ...
  const parsed = Object.fromEntries(names.map((name) => [name, Number(env[`TEAM_RECOMMENDATION_SCORE_${name.toUpperCase()}`])])) as unknown as ScoreWeights;
